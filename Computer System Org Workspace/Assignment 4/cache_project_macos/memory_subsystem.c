                   ...
                   uint8_t control, uint32_t *read_data)
{
  ...
  if(status == 0x0){
    ...
    l1_cache_access(address, write_data, control, read_data, &fake_status);
  }
}
...
void memory_handle_l1_miss(uint32_t address)
{
  ...
  l2_cache_access(address, NULL, 0x1, cache_line, &read_status);
  ...
  if(read_status == 0x0){
    num_l2_misses++;
    memory_handle_l2_miss(address, 0x1);
    uint8_t fake_status = 0;
    l2_cache_access(address, NULL, 0x1, cache_line, &fake_status);
  }
  ...
  uint8_t writeback_status = 0;
  l1_insert_line(address, cache_line, &evicted_writeback_address, evicted_writeback_data, &writeback_status);
  ...
  if(writeback_status == 0x1){
    uint8_t write_status = 0;
    l2_cache_access(evicted_writeback_address, evicted_writeback_data, 0x2, NULL, &write_status);
    if(write_status == 0x0){
      memory_handle_l2_miss(evicted_writeback_address, 0x2);
      uint8_t fake_status = 0;
      l2_cache_access(evicted_writeback_address, evicted_writeback_data, 0x2, NULL, &fake_status);
    }
  }
}
...
void memory_handle_l2_miss(uint32_t address, uint8_t control)
{
  ...
  if(control == 0x1){
    main_memory_access(address, NULL, 0x1, cache_line);
  }
  ...
  uint32_t evicted_writeback_data[WORDS_PER_CACHE_LINE];
  uint8_t writeback_status = 0;
  l2_insert_line(address, cache_line, &evicted_writeback_address, evicted_writeback_data, &writeback_status);
  ...
  if(writeback_status == 0x1){
    main_memory_access(evicted_writeback_address, evicted_writeback_data, 0x2, NULL);
  }
}
